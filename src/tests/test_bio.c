...
static char* _TestBioTypes() {
	...
	MU_ASSERT( "Cannot write varbuf", bbb_bio_WriteToFile_varbuf( vb, f, &chk ) );
	...
	MU_ASSERT( "bbb_bio_WriteToFile_...() failed", memcmp( x, data, sizeof( data ) ) == 0 );
	...
	xchk = 0;
	fseek( f, 0, SEEK_SET );
	MU_ASSERT( "Cannot read varbuf", bbb_bio_ReadFromFile_varbuf( &xvb, f, &xchk ) );
